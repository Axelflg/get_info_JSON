...
#Get the time (as a string) that a person spend in a place through Selenium
def looktime(name):
...
#Tranform the time as a string into an integer (through regular expressions) and get the average time (in minutes) 
def get_time(tiempo):    
    fminutos=0
    count=0
    #Get minutes
...
def main():
    ...
    for feature in data['features']:
        ...
        r = requests.get('https://maps.googleapis.com/maps/api/place/nearbysearch/json?location='+x+','+y+'&rankby=distance&distance=20&type='+str(cal)+'&key=AIzaSyBydM3PpubE1x3_Et1e_ApoFRujEvbUer8')  # Request to Google Places API
        # 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location='+x+','+y+'&radius=500&type='+str(cal)+'&key=AIzaSyCAJQUnW6GpmM5PmDa22kJuNFtOrwJTHhI'      Other possible request, focusing on the radio
        
		data2 = json.loads(r.content.decode("utf8"))  # JSON generated by the API request

        for out in data2['results']:
			#Get the firsts 5 results for each request to the Google Places API
            if cont <= 4 :
        ...
        feature['properties']['tiempo_medio'] = time  # Add the 'name' property
        print("\n Name: " + z)
        print("\n Tiempo: " + str(time))
